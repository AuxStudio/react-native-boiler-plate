...
const response = { user: 'bar' }; // we expect the response to have the user key

describe('signInAnonymously saga', () => {
  describe('When testing the saga without a nextAction and with a response from the api', () => {
    const it = sagaHelper(signInAnonymously(action));

    it('should have called the mocked API first', (result) => {
      expect(JSON.stringify(result)).toEqual(JSON.stringify(call(auth.signInAnonymously)));
  ...
  describe('When testing the saga with a nextAction and with a response from the api', () => {
    const it = sagaHelper(signInAnonymously(actionWithNextAction));
