...
import signInWithCredential from '..';
...
describe('signInWithCredential saga', () => {
  describe('When testing the saga without a nextAction and without a response from the api', () => {
    const it = sagaHelper(signInWithCredential(action));

    it('should have called the mocked API first', (result) => {
      expect(JSON.stringify(result)).toEqual(
        JSON.stringify(call(auth.signInWithCredential, action.payload.credential)),
      );
    });
  ...
  describe('When testing the saga with a nextAction and without a response from the api', () => {
    const it = sagaHelper(signInWithCredential(actionWithNextAction));
